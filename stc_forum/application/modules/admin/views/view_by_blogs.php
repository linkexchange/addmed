			...
			<div class="row">
				<div class="span12">
                	<div class="span12" style="margin:0px 0px 15px 0px; text-align:right; "> 
                    	<button onclick="javascript:goto('admin/articles/add')" class="btn btn-primary btn-large  icon-anchor"> Add Gallery Item</button>
                    </div>
                	<div class="widget">
                                        	...
                                        	<table class="table table-striped table-bordered">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="td-actions">
                                                            <a class="btn btn-small btn-success" href="<?php echo base_url()."admin/articles/edit/".$article['id']; ?>" title="Edit : <?php echo $article['articleTitle']; ?>" style="margin-bottom:5px;">
                                                                <i class="btn-icon-only icon-edit"> </i>
                                                            </a>
                                                            <a class="btn btn-danger btn-small" href="<?php echo base_url()."admin/articles/delete/".$article['id']; ?>" title="Delete : <?php echo $article['articleTitle']; ?>">
                                                    ...
                                                    if($count>$mod) :
                                                        ...
                                                        for($i=0;$i<=$count;$i++) :
                                                            ...
                                                            <?php if($this->uri->segment(4) && $this->uri->segment(5)) : ?>
                                                            	 <a class="btn btn-small btn-success page-<?php echo $inc; ?> <?php if($inc==$this->uri->segment(6))  echo "page-active"; else if(!($this->uri->segment(6)) && $inc==1)  echo "page-active";  ?>" href="<?php echo base_url()."admin/articles/index/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$inc; ?>" ><?php echo $inc; ?></a>
                                                            <?php else : ?>
                                                            	 <a class="btn btn-small btn-success page-<?php echo $inc; ?> <?php if($inc==$this->uri->segment(6))  echo "page-active"; else if(!($this->uri->segment(6)) && $inc==1)  echo "page-active";  ?>" href="<?php echo base_url()."admin/articles/index/0/0/".$inc; ?>" ><?php echo $inc; ?></a>
                                                            <?php endif; ?>
...
<script>
	function getBlogs(tid){
		if(tid){
			 $.ajax({
				url:base_url+"admin/articles/getTemplateBlogs/"+tid,
				//beforeSend: loadStartPub,
				//complete: loadStopPub,
				success:function(result){
					$("#setBlogsData").html(result);
	...
	function getDetails(bid){
		if(bid){
			...
			//alert(tid);
			$.ajax({
				url:base_url+"admin/articles/getArticlesByTemplatesAndBlog/"+tid+"/"+bid,
				//beforeSend: loadStartPub,
				//complete: loadStopPub,
				success:function(result){
