...
<script>
	$(document).ready(function(){
		...
		if(tid){
			 $.ajax({
				url:base_url+"admin/pages/getTemplatePages/"+tid+"/"+page,
				...
				<div class="span12">
                	<div class="widget add-blog"> 
                        <div class="span12 offset10"> 
                            <button onclick="javascript:goto('admin/pages/add')" class="btn btn-primary btn-large  icon-anchor">Add Page</button>
                        </div><!-- span12 offset10 -->
					</div><!-- widget add-blog -->
                                        ...
                                        <tbody>
                                            ...
                                            <?php  foreach($pages as $item) : ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a class="btn btn-small btn-success" href="<?php echo base_url()."admin/pages/edit/".$item['id']; ?>" title="Edit Post : <?php echo $item['title']; ?>">
                                                            <i class="btn-icon-only icon-edit"> </i>
                                                        </a>
                                                        <a class="btn btn-danger btn-small" href="<?php echo base_url()."admin/pages/delete/".$item['id']; ?>">
                                        ...
                                        if($count>$mod) :
                                            ...
                                            for($i=0;$i<=$count;$i++) :
                                                if(($i%$mod)==0) :
                                                    //echo $inc;
                                                ?>
                                                <a class="btn btn-small btn-success page-<?php echo $inc; ?> <?php if($inc==$this->uri->segment(5))  echo "page-active"; else if(!($this->uri->segment(5)) && $inc==1)  echo "page-active";  ?>" href="<?php echo base_url()."admin/pages/index/0/".$inc; ?>" ><?php echo $inc; ?></a>
                                                    <?php
                                                    $inc++;
                                                endif;
                                            endfor;
...
<script>
	function getDetails(lid){
			...
			if(lid){
			 $.ajax({
				url:base_url+"admin/pages/getTemplatePages/"+lid,
				//beforeSend: loadStartPub,
				//complete: loadStopPub,
				success:function(result){
					//alert(result);
